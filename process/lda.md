...
This technical note introduces a foundational architectural pattern for designing large industrial 
distributed control systems. [RTI](http://www.rti.com) proposed this first and  called it the
...
- Connectionless: Devices and applications do not rely on point-to-point connectivity. The connectivity 
  graph is dynamic with devices appearing/disappearing/reappearing across the system sharing a common databus.
  (Example: A networked patient monitoring system that continues to function as the patient is wheeled from one end of
  the hospital to another even through regions with no network connectivity)
